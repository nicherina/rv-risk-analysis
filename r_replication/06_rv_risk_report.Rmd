...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE,
                      fig.align = "center")
if (!file.exists("data/vehicles_clean.csv")) {
...
This analysis uses a dataset of **538,704 vehicle sales records** to build and validate a depreciation model. Risk classification and stress testing are then applied to a **lower age-band portfolio** — vehicles aged 12 years or under with a 40% guaranteed buyback — comprising `r if(exists("rs")) format(nrow(rs), big.mark=",") else "~267,000"` contracts.

**Three headline findings:**

| Finding | Detail |
|---|---|
| Age is the primary risk driver | Each additional year reduces market value by 13.3%; vehicles 9–12 years old already show material RV gaps |
| EVs carry a structural discount | EVs trade at a ~30% discount to equivalent ICE vehicles — directly eroding RV adequacy for any EV-heavy book |
| Portfolio stress is a base-case event | Expected loss exceeds €7.4M per 500-contract fleet across all 10,000 simulations — this is not a tail risk |
---
...
tibble(
  Metric = c("Total contracts (after cleaning)",
             "Contracts in lower age-band portfolio (age ≤ 12 yrs)",
  ...
  Value = c(
    ...
    paste0("€", format(round(mean(rs$price_actual, na.rm = TRUE)), big.mark = ",")),
...
The **RV gap** (guaranteed minus market) is the direct financial exposure per contract. Contracts are classified into four tiers. The analysis below applies to the **lower age-band portfolio** (vehicles aged ≤ 12 years) with a recalibrated guarantee of **40% of estimated new price**:
