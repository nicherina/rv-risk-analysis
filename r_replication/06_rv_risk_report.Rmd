...
| Systemic exposure | 80% of the portfolio falls in the "Critical" risk tier; expected loss exceeds €7.4M per 500-contract fleet |
...
tibble(
  Metric = c("Total contracts (after cleaning)", "Mean vehicle age",
             "Mean selling price", "Mean estimated new price",
             "Mean guaranteed RV (50% of new)", "Total portfolio exposure"),
  Value = c(
    format(nrow(df), big.mark = ","),
    paste0(round(mean(df$vehicle_age, na.rm = TRUE), 1), " years"),
    paste0("€", format(round(mean(df$price, na.rm = TRUE)), big.mark = ",")),
    "€56,583",
    "€28,292",
    "€7.66 billion"
  )
...
The **RV gap** (guaranteed minus market) is the direct financial exposure per contract. Contracts are classified into four tiers:

| Risk Tier | Definition | Count | Share |
|---|---|---|---|
| **Critical** | Large negative gap, high probability of loss | 377,920 | 80.2% |
| **High** | Moderate gap, likely loss | 52,317 | 11.1% |
| **Medium** | Small gap, possible loss | 20,405 | 4.3% |
| **Low** | Market value ≥ guaranteed RV | 20,696 | 4.4% |

> **80% of the portfolio sits in the Critical tier** — this is not isolated exposure; it reflects a structural mismatch between historical guarantee levels and current market realities.
